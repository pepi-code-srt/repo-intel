...
"""
    ...
    try:
        ...
        error_msg = "Gemini API Quota Exceeded (RESOURCE_EXHAUSTED)" if "RESOURCE_EXHAUSTED" in exc_str else f"API Error: {exc_str}"
        
        status_str = "QuotaExceeded" if "RESOURCE_EXHAUSTED" in exc_str else "Error"
        # Provide empty fallback findings so supervisor does not infinitely loop
        fallback = {
            "agent": "Engineering Analyzer", "score": None, "status": status_str,
            "strengths": [], "weaknesses": [error_msg],
